...
class Berita extends Controller
{
    ...
    public function proses(Request $request)
    {
        ...
        if(isset($_POST['hapus'])) {
            ...
            for($i=0; $i < sizeof($id_beritanya);$i++) {
                DB::table('blog')->where('id_blog',$id_beritanya[$i])->delete();
            }
        ...
        // PROSES SETTING DRAFT
            ...
            for($i=0; $i < sizeof($id_beritanya);$i++) {
                DB::table('blog')->where('id_blog',$id_beritanya[$i])->update([
        ...
        // PROSES SETTING PUBLISH
            ...
            if(is_array($id_beritanya)){
                ...
                return redirect('admin/berita')->with(['warning' => 'Pilih data terlebih dahulu']);
            }
    ...
    public function status_berita($status_berita)
    {
        ...
        // dd($status_berita);
        $myberita           = new Berita_model();
        $berita             = $myberita->status_blog($status_berita);
        $kategori    = DB::table('kategori')->orderBy('urutan','ASC')->get();
    ...
    public function tambah_proses(Request $request)
    {
        ...
        if(!empty($image)) {
            ...
            DB::table('blog')->insert([
                'id_kategori'       => $request->id_kategori,
                'id_user'           => Session()->get('id_user'),
                'slug_blog'       => $slug_berita,
                'judul_blog'      => $request->judul_berita,
                'isi'               => $request->isi,
                'jenis_blog'      => $request->jenis_berita,
                'status_blog'     => $request->status_berita,
    ...
    public function edit_proses(Request $request)
    {
        ...
        if(!empty($image)) {
            ...
            // END UPLOAD
            $slug_berita = str_slug($request->judul_berita, '-');
            DB::table('blog')->where('id_blog',$request->id_berita)->update([
                'id_kategori'       => $request->id_kategori,
                'id_user'           => Session()->get('id_user'),
                'slug_blog'       => $slug_berita,
                'judul_blog'      => $request->judul_berita,
    ...
    {
        if(Session()->get('username')=="") { return redirect('login')->with(['warning' => 'Mohon maaf, Anda belum login']);}
        DB::table('blog')->where('id_blog',$id_berita)->delete();
        return redirect('admin/berita')->with(['sukses' => 'Data telah dihapus']);
    }
}
